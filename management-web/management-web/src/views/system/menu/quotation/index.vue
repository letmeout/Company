<template>
  <div class="app-container">
    <header-title :need-border="true" style="margin-bottom: 20px;">
      <template slot="title"> 菜单管理 - 报价系统 </template>
    </header-title>
    <el-form v-show="showSearch" ref="queryForm" :model="queryParams" size="small" :inline="true">
      ...
      <el-form-item prop="status">
        <el-select v-model="queryParams.status" size="small" placeholder="请选择菜单状态" clearable>
          <el-option
...
export default {
  name: 'Menu',
  components: { Treeselect, headerTitle, IconSelect },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    async handleDelete(row) {
      try {
        ...
        const res = await delMenu(row.deptId)
        if (res && res.code == 200) {
