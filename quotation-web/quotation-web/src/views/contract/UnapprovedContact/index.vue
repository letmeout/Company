<template>
  <div>
  <a-card title="签约审批未通过" :headStyle="{ 'height': '66px' }">
    <template #extra>
    ...
    <a-table :columns="visibleColumns" :data-source="data" :pagination="pagination" row-key="key" :loading="loading"
      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <span v-else-if="column.dataIndex === 'actions'" style="text-align: left;">
          <a-button v-hasPermi="['quote:contract:rejected:details']" type="link" @click="handleDetail(record)" class="nomp">详情</a-button>
          <a-button v-hasPermi="['quote:contract:rejected:handleLost']" type="link" @click="handleUnqoute('discard', record)" class="nomp">丢单处理</a-button>
          <a-button v-hasPermi="['quote:contract:rejected:replay']" type="link" @click="handleUnqoute('reapply', record)" class="nomp">重新申请报价</a-button>
          <a-button v-hasPermi="['quote:contract:rejected:update']" type="link" @click="handleUpdate(record)" class="nomp">更新签约申请</a-button>
          <!-- <a-button v-hasPermi="['quote:contract:rejected:approve']" type="link" @click="handleApproval(record)">签约审批</a-button> -->
        </span>
      </template>
    </a-table>
  </a-card>
...
const loadOpportunities = async () => {
  ...
  try {
    const response = await fetchOpportunities('pageWithSignFail', params);
