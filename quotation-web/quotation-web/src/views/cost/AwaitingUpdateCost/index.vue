<template>
    <div>
    <a-card title="待更新成本报价" :headStyle="{ 'height': '66px' }">
        <template #extra>
        ...
        <a-table :columns="visibleColumns" :data-source="data" row-key="id" :pagination="pagination" :loading="loading"
            ...
            <template #bodyCell="{ column, record, index }">
                ...
                <span v-else-if="column.dataIndex === 'actions'" style="text-align: left;">
                    <a-button v-hasPermi="['quote:await:update:detail']" type="link" @click="viewDetails(record)"
                        class="nomp">商机详情</a-button>
                    <a-button v-hasPermi="['quote:await:update:quote:detail']" type="link" class="nomp"
                        @click="quoteDetails(record)">报价详情</a-button>
                    <a-button v-hasPermi="['quote:await:update:update']" type="link"
                        @click="updateDetails(record)" class="nomp" v-if="!record.quoteByOthers">更新报价</a-button>
                </span>
    ...
    <UpdateQuoteModal v-model:open="isUpdateModalVisible" :opportunity="selectedRecord" @close="handleUpdateCancel"
        :versionData="versionData" :isAwaitingModel="isAwaitingModel" />
...
import QuoteSelection from '../QuoteToCost/components/QuoteSelectionModal.vue'
...
const columns = ref([
    ...
    { title: '所属售前', dataIndex: 'preSaleName', sorter: (a, b) => a.preSaleName.localeCompare(b.preSaleName), width: '9%' },
    ...
    { title: '报价版本', dataIndex: 'currentVersion', key: 'currentVersion', width: '8%' },
...
const isAwaitingModel = ref(false)
...
const loadOpportunities = async () => {
    ...
    try {
        const response = await fetchOpportunities('pageAwaitUpdate', params);
...
function handleSelfSelectClose() {
    isQuoteSelectVisible.value = false;
    loadOpportunities();
}
