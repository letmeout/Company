<template>
    <div>
    <a-card title="暂停" :headStyle="{ 'height': '66px' }">
        <template #extra>
        ...
        <a-table :columns="visibleColumns" :data-source="data" :pagination="pagination" row-key="key" :loading="loading"
            ...
            <template #bodyCell="{ column, record, index }">
                ...
                <span v-if="column.dataIndex === 'actions'" style="text-align: left;">
...
const loadOpportunities = async () => {
    ...
    const filters = {
        ...
        name: '',
        category: '',
        statusList:["1","2", "3", "4", "5", "8", "9", "10", "12","13"],
        parentStatusList: ["00001304"],
    ...
    try {
        const response = await fetchOpportunities('pageWithSummary',params);
